...
if st.session_state.page == "Home":
    ...
    with st.expander("➕ Nowa potrawa"):
        ...
        if st.button("Utwórz kartę"):
            st.session_state.przepisy = pd.concat([st.session_state.przepisy, pd.DataFrame([{"Nazwa": np, "Skladnik": "Składnik", "Ilosc": 0}])], ignore_index=True); st.rerun()

    if not st.session_state.przepisy.empty:
        potrawy = sorted([p for p in st.session_state.przepisy['Nazwa'].unique().tolist() if str(p).strip()])
        if potrawy:
            wyb = st.selectbox("Edytuj przepis:", potrawy)
            mask = st.session_state.przepisy['Nazwa'] == wyb
            df_potr = st.session_state.przepisy[mask]
            
            for idx, row in df_potr.iterrows():
            ...
            if c_a.button("➕ Dodaj składnik"):
                st.session_state.przepisy = pd.concat([st.session_state.przepisy, pd.DataFrame([{"Nazwa": wyb, "Skladnik": "", "Ilosc": 0}])], ignore_index=True); st.rerun()
            if c_s.button("💾 ZAPISZ PRZEPIS", type="primary"): save_now(st.session_state.przepisy, "Przepisy")
    ...
    # Składniki z planu, których nie ma wcale w spiżarni
    ...
    if braki:
        ...
        st.success("Wszystko masz! Nie musisz iść na zakupy. 🎉")
