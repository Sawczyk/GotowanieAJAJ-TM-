...
# --- 1. KONFIGURACJA I ESTETYCZNA STYLIZACJA (CLEAN & AESTHETIC) ---
...
# --- 2. FUNKCJE POMOCNICZE I KONSTRUKCJA DANYCH ---
def to_num(val):
    try: 
...
def safe_load(ws, cols):
    """Pobiera dane i agresywnie naprawia strukturę, aby uniknąć KeyError."""
    try:
...
def save_now(df, ws_name):
    """Zapisuje dane i czyści cache Streamlit."""
    try:
        df_to_save = df.dropna(how='all').reset_index(drop=True)
        conn.update(worksheet=ws_name, data=df_to_save)
        st.cache_data.clear()
        st.toast(f"✅ Zaktualizowano arkusz: {ws_name}")
    except:
        st.error(f"Błąd zapisu {ws_name}. Sprawdź połączenie z Google Sheets.")
...
# --- PLANOWANIE Z DYNAMICZNYMI PORCJAMI ---
elif st.session_state.page == "Plan":
    st.subheader("📅 Planowanie i Skalowanie")
    ...
    for i, dzien in enumerate(dni_pl):
        with st.expander(f"{dzien} ({(start_date + timedelta(days=i)).strftime('%d.%m')})"):
            ...
            for typ in ["Śniadanie", "Obiad", "Kolacja"]:
                ...
                wyb = c_sel.selectbox(f"{typ}:", opcje, index=opcje.index(w_istn) if w_istn in opcje else 0, key=f"s_{k}")
                ...
                if wyb != "Brak":
                    ...
                    for _, r in przepis.iterrows():
                        nazwa_s = str(r['Skladnik']).lower().strip()
                        ile_skal = round(to_num(r['Ilosc']) * mnoznik, 2)
                        mam = st.session_state.spizarnia_df[st.session_state.spizarnia_df['Produkt'].str.lower().str.strip() == nazwa_s]['Ilosc'].apply(to_num).sum()
                        if mam < ile_skal:
            ...
            if suma_kcal_dnia > 0:
                st.markdown(f"<p class='kcal-tag'>🔥 Kalorie dnia: {int(suma_kcal_dnia)} kcal</p>", unsafe_allow_html=True)

# --- SPIŻARNIA ---
elif st.session_state.page == "Spizarnia":
    st.subheader("🏠 Stan Spiżarni")
    c_b, c_s = st.columns(2)
    if c_b.button("⬅ Menu"): st.session_state.page = "Home"; st.rerun()
    if c_s.button("💾 ZAPISZ STAN", type="primary"): save_now(st.session_state.spizarnia_df, "Spizarnia")
    ...
    with st.expander("➕ DODAJ PRODUKT"):
        with st.form("add_form", clear_on_submit=True):
            ...
            s = cs.checkbox("Produkt stały?")
            m = cm.number_input("Min. Ilość", 0.0)
...
# --- PRZEPISY (FIXED TYPES) ---
    ...
    with st.expander("➕ NOWA POTRAWA"):
        ...
        if st.button("Utwórz kartę"):
            if np:
                st.session_state.przepisy = pd.concat([st.session_state.przepisy, pd.DataFrame([{"Nazwa": np, "Skladnik": "Woda", "Ilosc": 0.0, "Kalorie": pk, "Porcje": pp}])], ignore_index=True)
                st.rerun()
    ...
    if not st.session_state.przepisy.empty:
        with st.expander("📝 EDYTUJ PRZEPISY"):
            ...
            st.session_state.przepisy.loc[mask, 'Porcje'] = cp.number_input("Porcje bazy:", value=float(to_num(st.session_state.przepisy.loc[mask, 'Porcje'].iloc[0])))
            ...
            if st.button("➕ Dodaj składnik"):
                st.session_state.przepisy = pd.concat([st.session_state.przepisy, pd.DataFrame([{"Nazwa": wyb_e, "Skladnik": "", "Ilosc": 0.0, "Kalorie": st.session_state.przepisy.loc[mask, 'Kalorie'].iloc[0], "Porcje": st.session_state.przepisy.loc[mask, 'Porcje'].iloc[0]}])], ignore_index=True); st.rerun()
...
# --- ZAKUPY ---
    ...
    for _, r in st.session_state.spizarnia_df.iterrows():
        ...
        if wymagane > mam: list_res.append({"Produkt": p_name.capitalize(), "Kupić": round(wymagane - mam, 2)})
        if p_name in needs: del needs[p_name]
    for s_name, ile in needs.items():
        if ile > 0: list_res.append({"Produkt": s_name.capitalize(), "Kupić": round(ile, 2)})

    if list_res:
        st.table(pd.DataFrame(list_res))
        txt = "ZAKUPY\n" + "\n".join([f"☐ {b['Produkt']}: {b['Kupić']}" for b in list_res])
        st.download_button("📥 Pobierz listę", txt, file_name="zakupy.txt", use_container_width=True)
